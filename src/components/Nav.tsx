...
import { Button, Chip, useDialog } from './Primitives'
import { ChevronDown, Close, Menu, Pin, Ticket } from './Icons'
...
export function Nav() {
  ...
  const { city, setCity, openPicker } = useCity()
  // A city in the URL (e.g. a shared /events link) wins over the remembered one.
  ...
  const selectCity = (name: string) => {
    setCity(name)
    setOpenMobile(false)
    navigate(name === ALL_CITIES ? '/events' : `/events?city=${encodeURIComponent(name)}`)
  }
    ...
    <>
      <header className="fixed inset-x-0 top-0 z-50 h-16 border-b border-line bg-white/95 backdrop-blur-md">
        <div className="wrap flex h-full items-center gap-4">
          ...
          {/* City selector — opens the full-screen city picker */}
          <button
            type="button"
            onClick={openPicker}
            className="hidden h-9 cursor-pointer items-center gap-2 rounded-lg border border-line bg-white px-3 text-sm font-medium text-ink transition-colors duration-150 hover:border-line-strong md:inline-flex"
          >
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <div className="relative hidden md:block">
              <SearchField
                ...
                onClear={() => setQ('')}
              >
                <SearchField.Group className="bg-surface-2">
                  <SearchField.SearchIcon />
                  <SearchField.Input className="w-44 xl:w-64" placeholder="Search events, artists, venues" />
          ...
          >
            ...
            <div className="border-b border-line p-4">
              <SearchField
                ...
                onClear={() => setQ('')}
              >
                <SearchField.Group className="h-11 bg-surface-2">
                  <SearchField.SearchIcon />
                  <SearchField.Input placeholder="Search events, artists, cities" />
                  <SearchField.ClearButton />
                </SearchField.Group>
              </SearchField>
            </div>
            ...
            <div className="border-b border-line p-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Chip active={selectedCity === ALL_CITIES} onClick={() => selectCity(ALL_CITIES)}>
                  All
                </Chip>
