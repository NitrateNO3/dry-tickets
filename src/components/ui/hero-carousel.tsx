// A full-bleed editorial hero driven by a filmstrip. Adapted from crafterui's Hero Carousel.
//
...
// every size below is a ratio of it.
//
// Local additions: `renderCta` (a slot beside the credit), `blurBackdrop` for small
// source images, and `wheelAxis`, so a hero at the top of a scrolling page can leave
// vertical wheel to the page.
...
export interface HeroCarouselItem {
  ...
  meta?: string[]
  /** CSS colour the background is graded to. @default "#8a8a8a" */
  accent?: string
}
...
export interface HeroCarouselProps {
  ...
  /** Which wheel axis steps the strip. 'x' leaves vertical scrolling to the page. @default "both" */
  wheelAxis?: 'both' | 'x'
  /** Soften the enlarged backdrop photo; useful when source images are small. @default false */
  blurBackdrop?: boolean
  /** Extra content beside the headline for the focused slide, e.g. a link. */
  renderCta?: (item: HeroCarouselItem, index: number) => React.ReactNode
  className?: string
}
...
export function HeroCarousel({
  ...
  // Quick, critically damped glide: settles fast without overshoot.
  const swing = reduced ? { duration: 0 } : { duration: 0.6, ease: [0.4, 0, 0.2, 1] as const }
  const spring = reduced ? { duration: 0 } : { type: 'spring' as const, stiffness: 220, damping: 30, mass: 0.8 }

  // The track is driven by a motion value rather than an `animate` prop so a
  // drag that starts mid-spring reads the real position.
  React.useEffect(() => {
    if (dragging) return
  ...
  React.useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (wheelAxis === 'x' && !horizontal) return
      const delta = horizontal ? e.deltaX : e.deltaY
      // Scroll chaining: once the strip is against an end, hand the gesture back to the page.
  ...
  return (
    ...
    >
      ...
      <AnimatePresence initial={false}>
        ...
        >
          {blurBackdrop ? (
            // Static: a blurred layer is rasterised once. Animating its scale would
            // re-run the blur on every frame, which is what made slides stutter.
            <img
              src={active.image}
              alt=""
              aria-hidden
              draggable={false}
              className="absolute inset-0 h-full w-full scale-[1.28] object-cover blur-[6px]"
            />
          ) : (
            <motion.img
              src={active.image}
      ...
      >
        <div className="flex w-full flex-wrap items-end gap-x-[6vw] gap-y-3">
          <AnimatePresence mode="popLayout" initial={false}>
            ...
            >
              {lines.map((line, i) => (
                ...
                <span key={i} className="-mb-[0.1em] block overflow-hidden pb-[0.14em]">
                  <motion.span
                    className="block"
                    initial={{ y: '110%' }}
                    animate={{ y: 0 }}
          ...
          {active.credit || renderCta ? (
            ...
            >
              {active.credit ? (
                <p className="font-mono uppercase tracking-[0.14em] opacity-80" style={{ fontSize: label }}>
                  {active.credit}
                </p>
              ) : null}
              {renderCta?.(active, index)}
            </motion.div>
          ) : null}
      ...
      <div className="absolute inset-x-0" style={{ top: `${STRIP_TOP * 100}%`, height: fullH }}>
        <motion.div
          ...
          onDragEnd={(_, info) => {
            setDragging(false)
            // Land on the card nearest the release, nudged by throw velocity.
            const thrown = x.get() + info.velocity.x * 0.08
        ...
        >
          {items.map((item, i) => (
            <motion.button
              ...
              // Fixed height, clipped to half when unfocused: animating the clip only
              // repaints, where animating height re-laid out every card each frame.
              style={{ width: cardW, height: fullH }}
              initial={false}
              animate={{ clipPath: i === index ? 'inset(0% 0% 0% 0%)' : 'inset(0% 0% 50% 0%)' }}
              transition={spring}
            >
              {/* Unfocused cards slide their photo up so the visible top half holds the face. */}
              <motion.img
                src={item.image}
                alt=""
                draggable={false}
